...
impl OverlayFS {
    ...
    async fn ensure_parent_dirs(&self, path: &str, uid: u32, gid: u32) -> Result<()> {
        ...
        for component in components.iter().take(components.len().saturating_sub(1)) {
            ...
            let new_stats =
                FileSystem::mkdir(&self.delta, current_parent_ino, component, 0o755, dir_uid, dir_gid)
                    .await?;
...
impl FileSystem for OverlayFS {
    ...
    async fn mkdir(&self, parent_ino: i64, name: &str, mode: u32, uid: u32, gid: u32) -> Result<Stats> {
        ...
        let mut stats = FileSystem::mkdir(&self.delta, delta_parent_ino, name, mode, uid, gid).await?;
...
mod tests {
    ...
    async fn test_overlay_lookup_after_mkdir_in_base_parent() -> Result<()> {
        ...
        let debug_stats = overlay.mkdir(target_stats.ino, "debug", 0o755, 0, 0).await?;
